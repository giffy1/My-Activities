...
DEBUG = True # indicates whether to print debugging logs

def _compute_formants(audio_buffer):
    """
...
def _compute_formant_features(window):
    """
    ...
    """
    return [1] # replace this dummy value

def _compute_pitch_contour(window):
    """
...
def _compute_delta_coefficients(window, N=2):
    """
    Computes the delta of the MFCC coefficients. See the equation in the assignment details.
    
    The _compute_mfcc() function returns a matrix of size n_frames x n_coefficients 
    (see function documentation above). With the default values, that is 79 X 13.
    
    The delta coefficients should also be a matrix of size O(n_frames) x n_coefficients. The 
    reason we use O-notation (on the order of) is because it depends how you handle edge 
    cases. Ignoring the edge cases will give a matrix of size (n_frames - 2N) x n_coefficients.
    
    However, we need it to be in a feature *vector*, not a matrix. So you will have 
    to flatten the matrix, giving you a O(n_frames * n_coefficients) array to return.
    
    The running-time is O(n_frames * N), so we generally want relatively small N. Default is N=2.
    First complete the algorithm nder the assumption that N is always 2. Then try to implement the 
    equation for any N.
...
def extract_features(window):
    ...
    x = np.append(x, _compute_formant_features(window))
    x = np.append(x, _compute_pitch_features(window))
    x = np.append(x, _compute_delta_coefficients(window))
